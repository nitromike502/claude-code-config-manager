...
/**
 * POST /api/copy/rule
 * Copy a rule configuration to a new location
 *
 * Request body:
 * - sourcePath: Absolute path to source rule file
...
async function copyRule(req, res) {
  ...
  try {
    ...
    const result = await copyService.copyRule({
      sourcePath,
      targetScope,
      targetProjectId,
    ...
    return res.status(statusCode).json({
      success: false,
      error: error.message || 'Unknown error occurred during rule copy'
    });
  }
}
